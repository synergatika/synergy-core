...
@Component({
  selector: 'sng-rich-editor-create',
  templateUrl: './rich-editor-create.component.html',
  styleUrls: ['./rich-editor-create.component.scss']
})
export class RichEditorCreateComponent implements OnInit, OnDestroy {
  ...
  checkImagesSize(currentFiles: EditorFile[]): boolean {
    ...
    currentFiles.forEach((el) => {
      if ((controls.description.value.indexOf(el.preview) > -1) && (el.file.size > this.validator.maxSize)) {
  ...
  checkImagesNumber(currentFiles: EditorFile[], previousFiles: string[]): boolean {
    ...
    if (previousFiles && previousFiles.length > 0) {
      previousFiles.forEach((el) => {
        if (controls.description.value.indexOf(el) > -1) {
          counter++;
        }
      })
    }
    if (currentFiles && currentFiles.length > 0) {
      currentFiles.forEach((el) => {
        if (controls.description.value.indexOf(el.preview) > -1) {
          counter++;
        }
      })
    }
  ...
  async uploadContentFiles() {
    ...
    message$.subscribe(
      (data) => {
         ...
         * Final Content Value
         */
        var content: string = controls.description.value;
        this.editorFiles.forEach((el) => {
          content = content.split(el.preview).join(el.url);
        });
        controls['description'].setValue(content);
